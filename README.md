...
## BLE 自动重连：为什么之前会"再也连不上"

设备离开一段时间（狗出门遛了半小时）再回来，之前有两条路会让重连协程永久卡死：

1. **`connect()` / `start_notify()` 不返回也不抛异常**。BlueZ 和 WinRT 都出现过，
   适配器状态错乱时握手会无限期挂起。协程卡在那一行，外层的重连循环再也不转。
   → 现在全部套 `asyncio.wait_for`（连接 20s、订阅 10s），并且用 `finally` 保证
   连接一定被释放，不留半开连接。
2. **广播根本收不到**。设备断开后系统蓝牙栈有时还留着陈旧的连接记录（Windows 的
   WinRT、Linux 的 BlueZ 都会），于是不再上报它的广播，`find()` 永远返回 None，
   循环静默地每秒空转下去。原有的扫描器看门狗管不了这种——另外几个设备和环境里的
   蓝牙设备一直在广播，"一条广播都收不到"的条件永远不成立。
   → 现在等超过 2 分钟没广播就重建扫描器（之后间隔翻倍，封顶 10 分钟），
   每 5 分钟打一行"仍未收到广播，已等待 N 分钟"，看日志就知道它还在努力。

另外：广播收得到但连不上，连续失败 3 次重建扫描器、6 次改用"按地址连"（绕开可能
已经失效的 BLEDevice 句柄）；扫描器重建有 30 秒冷却和互斥锁，四个设备同时判定
"该重建了"也只会真的重建一次。

另外还有一层：扫描器重建也救不回来的，是**整个适配器假死**（Windows 蓝牙栈被折腾到扫不到
任何设备，只有重启电脑才好，狗场 2 那台出过）。连着 3 次重建之间一条广播都没收到（连环境里
别人的手机都没有）就判定为这种，把适配器禁用再启用（`bt_adapter_reset.py`，等于拔插一次），
15 分钟内最多一次。只在 Windows + 管理员权限下做（计划任务起的录制满足）；`BT_RESET=0` 关掉。

## 日志

录制脚本的输出现在会**同时留一份到文件**：终端照常打印，`logs/record_multicam_<日期>.log`
里是带时间戳的同一份内容。跨天自动换新文件，默认保留 30 天。

```
logs/record_multicam_2026-09-08.log
```
...
用来干一件具体的事：**认清楚系统里的「摄像头 0」是哪个单间**。画面上每一路都写着
`camN` + 配对的狗名 + imu 编号 + 实时 Hz，对着屏幕认一遍就知道谁是谁。`--probe`
只打能力表，没有画面，回答不了这个问题。

这个模式**一个文件都不写**，连日期目录都不建——调试要反复开关，每次留一段 720p
视频和一堆 CSV 不但占地方，还会污染归档目录（归档是按目录扫的）。
预热也跳过（调试等不起 45 秒），预览强制开着（`PREVIEW=0` 在这个模式下不作数，
不看画面这个模式就没意义了）。
...
数据目录是 `data/multicam_multiimu/2026_9_9_gouchang`，NAS 上照搬同一个名字。

两个场地是两台机器、往同一个 NAS 传，日期目录本来是同一个 `2026_9_9`。文件不会
互相覆盖（文件名里的 imu 号全局唯一，时间戳精确到毫秒），但混在一个目录里，想确认
「狗场今天传全了没有」只能自己按 imu 号挑。

后缀写在场地配置里（`DAY_SUFFIX="_gouchang"`，老名字 `NAS_DAY_SUFFIX` 继续认），
**一律用 ASCII**——这个名字要一路穿过 `cygpath -w → robocopy → SMB → 后端容器的
挂载点`，中间任何一环编码没对齐就是个乱码目录，而且几天后才会从平台上发现。

以前不带后缀的目录原地不动，归档时两种名字都认（带后缀的存在就用它，否则退回老
名字），平台扫描用的是 `os.walk`，递归的，也都扫得到。
...
**这是完全可选的。** 不注册任务计划就等于没有它：录制脚本完全不知道这个文件存在，
不 import、不调用、不共享任何状态，录制行为跟以前一模一样。已经注册了想临时关掉，
设 `ARCHIVE_ENABLED=0`，或者在仓库根目录建一个空的 `.archive_disabled` 文件——
脚本一进来就退出，不用改任务计划也不用改代码。

（顺带一提，`cleanup_resampled_pairs.sh` 只多了一个 `-y` 参数给自动化用，不加 `-y`
手动跑还是照旧列清单等你确认。）
