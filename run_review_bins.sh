...
#
# 结果按 CSV_DIR 的文件夹名归档，比如 CSV_DIR=data/2026_7_18 会生成:
#   2026_7_18/                              推理结果目录（跟输入文件夹同名）
#     scratch_log_review_<lo>-<hi>.txt      推理输出日志，直接放这一层
#     2026_7_18/                            切分数据目录（同名，专门放剪出来的片段）
#       clips_<lo>-<hi>/                    每个区间对应的视频+CSV片段（见 clip_scratch_segments.py）
#
# 用法:
#   ./run_review_bins.sh
#   （先改下面几个变量，或者用环境变量覆盖，比如:
#    CSV_DIR=data/2026_7_18 MODEL=ml_rf.pkl ./run_review_bins.sh）
#
...
#
# RESULT_ROOT 可以改推理结果目录建在哪（默认当前目录下）；一般不用改。
...
# 去掉末尾的斜杠再取文件夹名，避免 CSV_DIR=data/2026_7_18/ 这种带尾斜杠的
# 输入取出来是空字符串。
BASE_NAME="$(basename "${CSV_DIR%/}")"
LOG_DIR="${RESULT_ROOT}/${BASE_NAME}"
CLIPS_ROOT="${LOG_DIR}/${BASE_NAME}"
...
if [ "$STEP" = "all" ] || [ "$STEP" = "clip" ]; then
    ...
    for bin in "${BINS[@]}"; do
        ...
        outdir="${CLIPS_ROOT}/${CLIP_OUT_PREFIX}_${lo}-${hi}"
    ...
    echo "全部区间处理完成，结果都在 ${LOG_DIR}/ 下："
    for bin in "${BINS[@]}"; do
        lo="${bin%-*}"
        hi="${bin#*-}"
        echo "  ${lo}~${hi}:  ${LOG_DIR}/scratch_log_review_${lo}-${hi}.txt  /  ${CLIPS_ROOT}/${CLIP_OUT_PREFIX}_${lo}-${hi}/"
    done
else
